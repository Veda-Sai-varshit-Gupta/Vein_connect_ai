...
const ICONS: Record<string, React.ReactNode> = {
  ...
  scheduling: <AlertCircle className="h-4 w-4 text-blue-500" />,
  ...
  return (
    <div className="space-y-5 max-w-xl">
      <div className="flex items-center justify-between">
        ...
        {unread > 0 && (
          <button onClick={markAllRead} className="text-xs font-semibold text-brand-500 hover:text-brand-600">
            Mark all read
          </button>
        )}
      </div>
      ...
      <div className="rounded-xl border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900 overflow-hidden">
        {notifications.length > 0 ? (
          <div className="divide-y divide-zinc-100 dark:divide-zinc-800">
            {notifications.map((n) => (
              <div
                ...
                className={`flex items-start gap-3 px-4 py-4 cursor-pointer transition-colors hover:bg-zinc-50 dark:hover:bg-zinc-900/50 ${!n.is_read ? "bg-brand-50/30 dark:bg-brand-950/10" : ""}`}
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2">
                    <p className={`text-xs font-bold ${n.is_read ? "text-zinc-600 dark:text-zinc-400" : "text-zinc-900 dark:text-white"}`}>
                      {n.title}
                    </p>
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center">
            <Bell className="h-10 w-10 text-zinc-300 dark:text-zinc-700 mb-3" />
