...
import { Bell, AlertTriangle, Calendar, CheckCircle, Loader2 } from "lucide-react";
            ...
            {notifications.map((n) => (
              <div key={n.id} onClick={() => !n.is_read && markRead(n.id)} className={`flex items-start gap-3 px-4 py-4 cursor-pointer transition-colors hover:bg-zinc-50 dark:hover:bg-zinc-900/50 ${!n.is_read ? "bg-brand-50/30 dark:bg-brand-950/10" : ""}`}>
                <div className="mt-0.5">
                  {n.type === "emergency" ? <AlertTriangle className="h-4 w-4 text-red-500" /> : n.type === "non_response" ? <Bell className="h-4 w-4 text-amber-500" /> : n.type === "expense" ? <Calendar className="h-4 w-4 text-blue-500" /> : <CheckCircle className="h-4 w-4 text-emerald-500" />}
                </div>
