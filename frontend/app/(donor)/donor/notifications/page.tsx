...
import { Bell, CheckCircle, Loader2, Heart } from "lucide-react";
            ...
            {notifications.map((n) => (
              <div key={n.id} onClick={() => !n.is_read && markRead(n.id)} className={`flex items-start gap-3 px-4 py-4 cursor-pointer transition-colors hover:bg-zinc-50 dark:hover:bg-zinc-900/50 ${!n.is_read ? "bg-brand-50/30 dark:bg-brand-950/10" : ""}`}>
                <div className="mt-0.5">
                  {n.type === "donation_request" ? <Heart className="h-4 w-4 text-brand-500" /> : n.type === "reward" ? <Bell className="h-4 w-4 text-amber-500" /> : <CheckCircle className="h-4 w-4 text-emerald-500" />}
                </div>
