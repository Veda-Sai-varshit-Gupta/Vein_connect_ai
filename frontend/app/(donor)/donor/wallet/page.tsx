...
import { Wallet, ArrowUpRight, ArrowDownLeft, Plus, Loader2, IndianRupee, X } from "lucide-react";
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        ...
        setWallet(wRes.data);
        setTransactions(Array.isArray(tRes.data) ? tRes.data : tRes.data?.items || []);
      } catch {
        setWallet(null);
        setTransactions([]);
      } finally {
  ...
  return (
    <div className="space-y-6 max-w-xl">
      <div>
        ...
        <p className="text-xs text-zinc-500 mt-0.5">Earnings, reimbursements and balance.</p>
      </div>

      {/* Balance Card */}
      <div className="rounded-2xl bg-gradient-to-br from-emerald-500 via-teal-600 to-brand-600 p-6 text-white shadow-lg">
        <div className="flex items-center justify-between">
          <div>
            ...
            <div className="flex items-baseline gap-1 mt-2">
              ...
              <span className="text-4xl font-black">{(wallet?.balance ?? 0).toLocaleString("en-IN")}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900 overflow-hidden">
        <div className="border-b border-zinc-100 dark:border-zinc-800 px-4 py-3">
          <h2 className="text-xs font-bold uppercase tracking-wider text-zinc-600 dark:text-zinc-400">Transactions</h2>
        </div>
        {transactions.length > 0 ? (
          <div className="divide-y divide-zinc-100 dark:divide-zinc-800">
            {transactions.map((tx) => (
              <div key={tx.id} className="flex items-center gap-4 px-4 py-3">
                <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full ${tx.type === "credit" ? "bg-emerald-50 dark:bg-emerald-950/20" : "bg-red-50"}`}>
                  {tx.type === "credit" ? <ArrowDownLeft className="h-4 w-4 text-emerald-500" /> : <ArrowUpRight className="h-4 w-4 text-red-400" />}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-semibold text-zinc-900 dark:text-white truncate">{tx.description}</p>
                  <p className="text-[10px] text-zinc-400 mt-0.5">{format(parseISO(tx.created_at), "dd MMM yyyy")}</p>
                </div>
